...
inline bool CShape::InBounds(int x, int y) const
{
...
inline bool CShape::InBounds(int x, int y, int b) const
{
    // Is given pixel address valid?
    return (0 <= x && x < width &&
            0 <= y && y < height &&
...
inline const void* CImage::PixelAddress(int x, int y, int band) const
{
    // This could also go into the implementation file (CImage.cpp):
    return (void *) &m_memStart[y * m_rowSize + x * m_pixSize + band * m_bandSize];
}
...
template <class T>
inline const T& CImageOf<T>::Pixel(int x, int y, int band) const
{
    return *(T *) PixelAddress(x, y, band);
}
