...
struct CShape
{
    ...
    bool InBounds(int x, int y) const;            // is given pixel address valid?
    bool InBounds(int x, int y, int band) const;  // is given pixel address valid?
    bool operator==(const CShape& ref) const;     // are two shapes the same?
    bool SameIgnoringNBands(const CShape& ref) const; // " ignoring the number of bands?
    bool operator!=(const CShape& ref) const;     // are two shapes not the same?
};
...
public:
    ...
    const CShape Shape(void) const  { return m_shape; }
